...
func main() {
	...
	healthChecker := health.NewChecker(masterDB.GetDB(), redisClient, minioClient, cfg, "1.0.0")

	// Initialize metrics (blank identifier since Handler uses global prometheus registry)
	_ = metrics.NewMetrics("platform-api", "1.0.0")
	...
	platform.SetupRoutes(router, masterDB.GetDB(), cfg, redisClient)
	...
	go func() {
		log.Printf("Starting Platform API server on %s", srv.Addr)
